...
`app/data/countries.py` 不維護自己的國家清單，而是架在國際標準資料上：

| 資料 | 來源 | 涵蓋 |
| --- | --- | --- |
| 國家代碼與名稱 | `pycountry`（ISO 3166-1） | 242 國可用 |
| 行政區（含第二層） | `pycountry`（ISO 3166-2） | 5046 個 |
| 在地化國名 | `babel`（CLDR） | 各語系 |
| 幣別 | `babel`（CLDR → ISO 4217） | 全部 |
| 電話碼與號碼驗證 | `phonenumbers`（libphonenumber） | 全部 |
| 時區 | `pytz`（IANA zone.tab） | 全部 |

**支援一個新國家不需要改任何程式碼或資料。** 原本這裡是一份手寫的 12 國字典，
接烏干達時就卡住了：沒有 +256、沒有 UGX、沒有 Africa/Kampala，也沒有行政區。

檔案裡仍保留幾張小的覆蓋表，補標準資料沒有的東西：英制國家（US/LR/MM）、
多時區國家的首選時區、中日文的行政區名稱（ISO 只給羅馬字）、
以及中文語系的行政區標籤（縣市 / 都道府県）。覆蓋是加法，沒補到就用標準資料。

**行政區是階層的。** 有些國家的一級太粗——烏干達的一級是 4 個 Region，
實際的行政單位是底下 135 個 District。這種國家 `has_second_level` 為 true，
前端可以用 `?parent=UG-E` 往下鑽。使用者填哪一層都接受。
